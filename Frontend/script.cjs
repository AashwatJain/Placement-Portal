...
const targetDir = 'c:\\Users\\Aniket\\OneDrive\\Desktop\\Coding\\PlacementPortal\\Placement-Portal\\Frontend\\src';
...
function replaceColorsInFile(filePath) {
  ...
  content = content.replace(/dark:bg-slate-900\/(\d+)/g, 'dark:bg-[#1A0F08]/$1');
  content = content.replace(/dark:bg-slate-900/g, 'dark:bg-[#1A0F08]');
  content = content.replace(/dark:bg-slate-800\/(\d+)/g, 'dark:bg-[#2A1810]/$1');
  ...
  // Dark mode texts
  content = content.replace(/dark:text-slate-100/g, 'dark:text-brand-beige-100');
  ...
  content = content.replace(/blue-200/g, 'brand-amber-500/30');
  content = content.replace(/blue-300/g, 'brand-amber-500/40');
  content = content.replace(/blue-400/g, 'brand-amber-500');
  ...
  content = content.replace(/blue-800/g, 'brand-amber-700');
...
walkDir(targetDir);
console.log('✅ Done replacing colors in entire src directory.');
